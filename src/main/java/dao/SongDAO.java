...
import ConnectionToDatabase.ConnectionDatabase;
import dto.Pageable;
import model.Author;
...
public class SongDAO extends ConnectionDatabase {
    ...
    private final String SELECT_SONG_BY_ID = "SELECT song.*, author.`name` as name_author , category.`name` as type, singer.`name` as name_singer,singer.`image_singer` as imgsinger  FROM song  \n" +
    ...
    private final String SELECT_ALL_SONG = "SELECT song.*, author.`name` as name_author , category.`name` as type, singer.`name` as name_singer,singer.`image_singer` as image_singer   FROM song  \n" +
            "LEFT JOIN author  ON song.id_author = author.id\n" +
            "LEFT JOIN category  ON song.id_category = category.id\n" +
    ...
    public Song findById(int id) {
        try (Connection connection = getConnection();
             ...
             PreparedStatement preparedStatement = connection
                     .prepareStatement(SELECT_SONG_BY_ID);) {
            System.out.println(preparedStatement);
            ...
            while (rs.next()) {
                ...
                int singerID = rs.getInt("id_singer");
                String singers = rs.getString("name_singer");
                String image_singer = rs.getString("imgsinger");
                Singer singer = new Singer(singerID, singers,image_singer);
                String duration = rs.getString("duration");
                return new Song(idSong, name, authors, category, singer, song, image,duration);
            }
        } catch (SQLException e) {
            System.out.println(e.getMessage());
        }
